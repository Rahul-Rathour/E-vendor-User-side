...
const Myorders = () => {
  ...
  const getStatusProps = (status) => {
    switch (status.toLowerCase()) {
      case "delivered":
        return { icon: <FaCheckCircle className="text-green-600 text-xl" />, color: "text-green-600 bg-green-100" };
      case "shipped":
        return { icon: <FaTruck className="text-yellow-600 text-xl" />, color: "text-yellow-600 bg-yellow-100" };
      default:
        return { icon: <FaClock className="text-gray-600 text-xl" />, color: "text-gray-600 bg-gray-100" };
    }
  };
